...
export default function AdminLayout({ children }) {
  ...
  // Ocultar "Usuarios" si no es admin principal
  const visibleItems = navItems.filter(item => item.to !== "/admin/users" || admin.isMain);

  return (
    <div style={{ minHeight: "100vh", background: "#0a0a0a", display: "flex" }}>
      <aside style={{ width: "240px", background: "#0f0f0f", borderRight: "1px solid #1e1a14", display: "flex", flexDirection: "column", position: "fixed", height: "100vh" }}>
        <div style={{ padding: "28px 24px", borderBottom: "1px solid #1e1a14" }}>
          ...
          {admin.isMain && (
            <div style={{ fontFamily: "'Lato', sans-serif", fontSize: "9px", letterSpacing: "2px", color: "#c9a84c", textTransform: "uppercase", marginTop: "4px" }}>
              ★ Admin principal
            </div>
          )}
        </div>
